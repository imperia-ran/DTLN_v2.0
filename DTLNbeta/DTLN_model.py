...
from keras.layers import Activation, Dense, LSTM, Dropout, Lambda, Input, Multiply, Layer, Conv1D
...
import numpy as np
# 在DTLN_aecmodel类的compile_model方法中添加混合精度
...
class audio_generator():
    ...
    def create_generator(self):
        ...
        for file in self.file_names:
            ...
            suffix = str.split(file, 'fileid_')[-1]
            # 数据集压缩要用
            # id = ''
            # if 'reverse' in file:
            #     id = suffix
            # else:
            #     id = str.split(file, '_')[-1]
            mic_in, fs_1 = sf.read(os.path.join(self.path_to_mic, 'nearend_mic_fileid_'+suffix)) #Brian Modify
            lpb_in, fs_2 = sf.read(os.path.join(self.path_to_lpb, 'farend_speech_fileid_'+suffix)) #Brian Modify
            speech, fs_3 = sf.read(os.path.join(self.path_to_s1,  'nearend_speech_fileid_'+suffix)) #Brian Modify
            #print('nearend_mic_fileid_'+suffix, 'farend_speech_fileid_'+suffix, 'nearend_speech_fileid_'+suffix)
...
class DTLN_aecmodel():
    ...
    def build_DTLN_model(self, norm_stft=False):
        ...
        #Brian Modify
        # mic_norm =InstantLayerNormalization()(
        #     tf.keras.layers.Lambda(lambda x: tf.math.log(x + 1e-7))(mic_mag)
        # )
        # lpb_norm=InstantLayerNormalization()(
        #     tf.keras.layers.Lambda(lambda x: tf.math.log(x + 1e-7))(lpb_mag)
        # )
        mic_norm =InstantLayerNormalization()(ops.log(mic_mag + 1e-7))
        lpb_norm=InstantLayerNormalization()(ops.log(lpb_mag + 1e-7))
        ...
        estimated_mag = SafeMultiply()([mic_mag, mask_1]) #zg
        ...
        estimated = SafeMultiply()([encoded_frames_mic, mask_2]) 
    ...
    def build_DTLN_model_stateful(self, norm_stft=False):
        ...
        time_dat =  [Input(batch_shape=(1, self.blockLen)),Input(batch_shape=(1, self.blockLen))] #Brian Modify
        mic_data=time_dat[0]
        lpb_data=time_dat[1]
        # calculate STFT
        lpb_time=Lambda(self.framegenerator)(lpb_data)
        ...
        # multiply mask with magnitude
        estimated_mag = SafeMultiply()([mic_mag, mask_1])
        #estimated_mag = Multiply()([mic_mag, mask_1])
        # transform frames back to time domain
        estimated_frames_mic=  Lambda(self.ifftLayer)([estimated_mag,mic_angle])
        # encode time domain frames to feature domain
    ...
    def create_tf_lite_model(self, weights_file, target_name, use_dynamic_range_quant=False):
        ...
        model1_out=SafeMultiply()([mic_mag, mask_1])
        ...
        mask_2, states_out_2 = self.seperation_kernel_with_states(self.encoder_size, 
                                                    ...
                                                    states_in_2)
        # multiply encoded frames with the mask
        estimated = SafeMultiply()([encoded_frames, mask_2]) 
        ...
        if use_dynamic_range_quant:
            ...
            converter.target_spec.supported_types = [tf.float16] #Brian Add fp16 quantize
    ...
    def train_model(self, runName, path_to_train_mic, path_to_train_lpb,path_to_train_speech, \
        ...
        print('len_in_samples', len_in_samples)
...
class SafeMultiply(Layer):
    def __init__(self, **kwargs):
        super(SafeMultiply, self).__init__(**kwargs)

    def call(self, inputs):
        x, y= inputs
        #限制数值范围防止溢出
        x= tf.clip_by_value(x,-1e4,1e4)
        y= tf.clip_by_value(y,-1e4,1e4)
        return x*y
...
if __name__=='__main__':
    ...
    if sys.argv[1] == 'train':
        # For training 
        dataset_root = '/home/brian/Desktop/npu_deploy/train_model/dataset/'
        #dataset_root = '../dataset_mini/'
        micpath = dataset_root+'train/nearend_mic_signal/'
        lpbpabth = dataset_root+'train/farend_speech/'
        tarpath = dataset_root+'train/nearend_speech/'
        mictest = dataset_root+'val/nearend_mic_signal/'
        lpbtest = dataset_root+'val/farend_speech/'
        speechtest = dataset_root+'val/nearend_speech/'

        aecmodel=DTLN_aecmodel()
        aecmodel.build_DTLN_model()
        aecmodel.compile_model()
        aecmodel.train_model('dtln_aecmodel_blockLen{}_blockShift_{}'.format(aecmodel.blockLen, aecmodel.block_shift),micpath,lpbpabth,tarpath,mictest,lpbtest,speechtest)

    elif sys.argv[1] == 'contrain':
        # For training 
        dataset_root = '/home/brian/Desktop/npu_deploy/train_model/dataset/'
        #dataset_root = '../dataset_mini/'
        micpath = dataset_root+'train/nearend_mic_signal/'
        ...
        aecmodel=DTLN_aecmodel()
        aecmodel.build_DTLN_model()
        aecmodel.model.load_weights(sys.argv[2]+'.weights.h5')
        aecmodel.compile_model()
        aecmodel.train_model('dtln_aecmodel_blockLen{}_blockShift_{}'.format(aecmodel.blockLen, aecmodel.block_shift),micpath,lpbpabth,tarpath,mictest,lpbtest,speechtest)
        ...
        path = sys.argv[2]+'/'
        h5_names = fnmatch.filter(os.listdir(path), '*.weights.h5')
        # For export model
        aecmodel=DTLN_aecmodel()
        weightsfile=path+h5_names[0]
        targetname=path+h5_names[0].split('.')[0]
        #aecmodel.create_onnx_model(weightsfile,targetname)
        aecmodel.create_tf_lite_model(weightsfile,targetname,True)

    elif sys.argv[1] == 'param':
        aecmodel=DTLN_aecmodel()
        aecmodel.build_DTLN_model_stateful()
        aecmodel.model.load_weights(sys.argv[2]+'.weights.h5')
        keras.saving.save_model(aecmodel.model, sys.argv[2]+'.h5')

    else:
        print("param err")

# pip install tensorflow[and-cuda]==2.19.0
# python DTLN_model.py train
# python DTLN_model.py convert ./{name}
# python DTLN_model.py contrain ./models_dtln_aecmodel_blockLen512_blockShift_128_lstm_512_v1/dtln_aecmodel_blockLen512_blockShift_128