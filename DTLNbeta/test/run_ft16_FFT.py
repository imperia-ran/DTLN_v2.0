...
def process_file(interpreter_1, interpreter_2, audio_file_name, out_file_name):
    ...
    block_len = 512 #Brian Modify
    block_shift = 256 #Brian Modify
    ...
    for idx in range(num_blocks):
        #start_time = time.time() #Brian Add 
        ...
        # create magnitude
        in_phase = np.angle(in_block_fft) #Brian Add!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!解决音量不对问题
        ...
        print("input_0", input_details_1[0]["shape"]) #Brian Add
        print("input_1", input_details_1[1]["shape"]) #Brian Add
        print("input_2", input_details_1[2]["shape"]) #Brian Add
        print("in_mag", in_mag.shape) #Brian Add
        print('lpb_mag', lpb_mag.shape) #Brian Add
        print('states_1', states_1.shape) #Brian Add

        interpreter_1.set_tensor(input_details_1[0]["index"], in_mag)
        interpreter_1.set_tensor(input_details_1[1]["index"], lpb_mag)
        interpreter_1.set_tensor(input_details_1[2]["index"], states_1) #fix
        # run calculation
        ...
        estimated_complex = np.squeeze(out_mask)  * np.exp(1j * in_phase) #Brian Add!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!解决音量不对问题
        estimated_block = np.fft.irfft(estimated_complex) #Brian Add!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!解决音量不对问题
        ...
        print("input_0", input_details_2[0]["shape"]) #Brian Add
        print("input_1", input_details_2[1]["shape"]) #Brian Add
        print("input_2", input_details_2[2]["shape"]) #Brian Add
        print("in_lpb", in_lpb.shape) #Brian Add
        print('estimated_block', estimated_block.shape) #Brian Add
        print('states_2', states_2.shape) #Brian Add

        interpreter_2.set_tensor(input_details_2[1]["index"], states_2) #fix
        ...
        #end_time = time.time()
        #print("round:", idx, "time:", (end_time - start_time)*1000)
...
# 解决声音失真问题
# python run_ft16_FFT.py -i ./input/ -o ./output/ -m ../dtln_aecmodel_blockLen512_blockShift_256